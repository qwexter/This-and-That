...
class TasksAuthTest {
    ...
    fun `GET tasks returns only caller's tasks`() = todoApp(
        ...
        val response = client.get("tasks") { header("X-Forwarded-User", "user-a") }
    ...
    fun `GET tasks returns empty list when caller has no tasks`() = todoApp(
        ...
        val tasks = client.get("tasks") { header("X-Forwarded-User", "user-b") }.body<List<ActiveTask>>()
    ...
    fun `GET task by id returns 404 when task belongs to another user`() = todoApp(
        ...
        val response = client.get("tasks/task-a") { header("X-Forwarded-User", "user-b") }
    ...
    fun `GET task by id returns task when caller is owner`() = todoApp(
        ...
        val response = client.get("tasks/task-a") { header("X-Forwarded-User", "user-a") }
        assertEquals(HttpStatusCode.OK, response.status)
    ...
    fun `PATCH task returns 404 when task belongs to another user`() = todoApp(
        ...
        val response = client.patch("tasks/task-a") {
            header("X-Forwarded-User", "user-b")
            contentType(ContentType.Application.Json)
            setBody(UpdateTask(status = ApiTaskStatus.Done))
        }
        assertEquals(HttpStatusCode.NotFound, response.status)
    ...
    fun `PATCH task succeeds when caller is owner`() = todoApp(
        ...
        val response = client.patch("tasks/task-a") {
            header("X-Forwarded-User", "user-a")
            contentType(ContentType.Application.Json)
            setBody(UpdateTask(status = ApiTaskStatus.Done))
        }
        assertEquals(HttpStatusCode.OK, response.status)
    ...
    fun `DELETE task returns 404 when task belongs to another user`() = todoApp(
        ...
        val response = client.delete("tasks/task-a") { header("X-Forwarded-User", "user-b") }
        assertEquals(HttpStatusCode.NotFound, response.status)
    }
    ...
    fun `DELETE task succeeds when caller is owner`() = todoApp(
        ...
        val response = client.delete("tasks/task-a") { header("X-Forwarded-User", "user-a") }
    ...
    fun `POST tasks creates task owned by caller`() = todoApp(
        ...
        // user-a sees it
        val aTasks = client.get("tasks") { header("X-Forwarded-User", "user-a") }.body<List<ActiveTask>>()
        assertEquals(1, aTasks.size)

        // user-b does not
        val bTasks = client.get("tasks") { header("X-Forwarded-User", "user-b") }.body<List<ActiveTask>>()
